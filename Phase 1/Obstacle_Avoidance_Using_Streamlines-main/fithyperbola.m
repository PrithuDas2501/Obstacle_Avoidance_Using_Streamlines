...
function hfit  =  fithyperbola(x, y)
...
% The following has not been tested; just provided from the source
% documentation. 
% alternative for orthog hyperbola
% function [orthConic, fitStd]  =  fitOrtHyperbola( x,y )
% %%(c) Paul O'Leary 2003
% % It is assumed that x and y are columnvectors of the same length
% %
% x2 = x.^2;
% y2 = y.^2;
% xy = x.*y;
...
% %
% result = V(:,end)';
% %%Convert it to standard form for a conic
% with all 6 coefficients
% % NOTE: result(1) is repeated
% %
% orthConic = [-result(1), result(2), result
% (1), result(3:end)];
% %%Determine the standard deviation of the residual.
% %
% fitStd = S(end,end)/sqrt(length(x));
% end
